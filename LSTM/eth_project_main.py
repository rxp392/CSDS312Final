...
def plot_data(df):
    ...
    plt.savefig("ETH_price_data.png")
    plt.clf()
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("ETHUSDT_Binance_futures_data_hour.csv", skiprows=1, low_memory=False)
    df_copy = df.copy()
    ...
    df_work = df_copy[['dt', 'hour', 'week_day', 'close', 'Volume ETH']]

    # normalize the data
    X = df_work[['hour', 'week_day', 'Volume ETH', 'close']]
    Y = df_work[['close']]
    ...
    plt.title('ETH Training and validation loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig("ETH_Training_and_Validation_loss.png")
    plt.clf()
    ...
    fig = px.line(result, x=result.index.values, y="price", color="type", title="Ethereum Price Prediction")
    fig.write_image("eth_results.png")
